...
namespace WEB_API.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("[action]")]
        public async Task<IActionResult> AddUser(string name, string account, string password, string role)
        {
            var result = await _User_Service.AddUser(name, account, password, role);
        ...
        {
            var result = await _User_Service.UpdateUser(user.id, user.name, user.account, user.password, user.role);
            switch (result.success)
            {
                case true:
                    return Ok(result);
